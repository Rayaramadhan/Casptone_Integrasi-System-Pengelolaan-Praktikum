  ...
  <main class="flex-1 w-full">
    <section class="max-w-7xl mx-auto px-4 lg:px-6 py-10">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <h1 class="text-2xl sm:text-3xl font-semibold text-slate-900">📋 Review Laporan Praktikum</h1>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-[0_8px_24px_rgba(15,23,42,0.06)] border border-slate-100 p-4 md:p-5 hover:shadow-lg transition">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-xs text-slate-500 mb-1">Revisi</p>
      ...
      <div class="bg-white rounded-3xl shadow-[0_14px_40px_rgba(15,23,42,0.08)] border border-slate-100 overflow-hidden mb-6">
        ...
        <div class="px-6 py-6">
          <form method="GET" action="{{ route('laboran.reports.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
            ...
            <div>
              ...
              <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul atau nama asprak..." class="w-full border-slate-200 rounded-xl shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm">
            </div>
        ...
        <div class="px-6 py-6">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                          <div class="h-8 w-8 rounded-full bg-teal-500 text-white flex items-center justify-center text-xs font-semibold">
                      ...
                      <td class="px-6 py-4">
                        ...
                        @if($report->description)
                          <div class="text-sm text-slate-500 mt-1">{{ Str::limit($report->description, 40) }}</div>
                        @endif
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center gap-2">
                          <a href="{{ route('laboran.reports.show', $report) }}" 
                          ...
                          @if($report->status === 'pending')
                            <form method="POST" action="{{ route('laboran.reports.approve', $report) }}" class="inline">
                              @csrf
                              <button type="submit" 
                                      onclick="return confirm('Yakin ingin menyetujui laporan ini?')"
                                      class="inline-flex items-center justify-center h-8 w-8 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition" 
                                      title="Approve">
                                ✅
                              </button>
                            </form>
          ...
          @else
            <div class="text-center py-16">
              ...
              <p class="text-slate-500 text-lg mb-4">Belum ada laporan yang masuk.</p>
            </div>
          @endif
        </div>
      </div>
