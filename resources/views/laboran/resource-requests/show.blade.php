...
<head>
    ...
    <title>Review Detail Kebutuhan - SIAP</title>
    ...
    <header class="bg-white border-b border-teal-100 shadow-sm sticky top-0 z-50">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('laboran.resource-requests.index') }}" class="text-sm text-slate-600 hover:text-teal-600 transition">
                    ← Kembali
                </a>
            </div>
        </div>
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-6 bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-red-700 mt-1">
                    ...
                    Prioritaskan review permintaan ini.
                </p>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-3xl shadow-[0_14px_40px_rgba(15,23,42,0.08)] border border-slate-100 p-8">
                    ...
                    <div class="mt-4 flex items-center gap-2 text-xs text-slate-500">
                        ...
                        <span class="font-semibold text-slate-700">{{ $resourceRequest->reviewer->name }}</span>
                        <span>•</span>
                        <span>{{ $resourceRequest->reviewed_at->format('d M Y, H:i') }}</span>
                    </div>
                </div>
                @endif
            ...
            <div class="space-y-6">
                ...
                <!-- Action Buttons -->
                <div class="bg-white rounded-3xl shadow-[0_14px_40px_rgba(15,23,42,0.08)] border border-slate-100 p-6">
                    <h3 class="text-sm font-bold text-slate-900 mb-4">Review Actions</h3>
                    
                    @if($resourceRequest->status === 'pending')
                    <div class="space-y-3">
                        <!-- Approve -->
                        <a href="{{ route('laboran.resource-requests.approveForm', $resourceRequest) }}" class="block w-full px-4 py-2.5 bg-green-500 text-white rounded-lg hover:bg-green-600 transition text-sm font-medium text-center">
                            ✅ Setujui Permintaan
                        </a>

                        <!-- Reject -->
                        <a href="{{ route('laboran.resource-requests.rejectForm', $resourceRequest) }}" class="block w-full px-4 py-2.5 bg-red-500 text-white rounded-lg hover:bg-red-600 transition text-sm font-medium text-center">
                            ❌ Tolak Permintaan
                        </a>

                        <!-- Back to List -->
                        <a href="{{ route('laboran.resource-requests.index') }}" class="block w-full px-4 py-2.5 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition text-sm font-medium text-center">
                            ← Kembali ke Daftar
                        </a>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-slate-100">
                        <p class="text-xs text-slate-500">
                            💡 <strong>Wajib:</strong> Berikan feedback yang jelas agar Asisten memahami keputusan Anda.
                        </p>
                    </div>
                    @else
                    <div class="space-y-3">
