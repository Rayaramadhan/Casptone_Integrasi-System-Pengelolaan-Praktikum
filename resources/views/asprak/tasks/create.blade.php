...
<head>
  ...
  <title>SIAP - Tambah Tugas Baru</title>
  ...
  <main class="flex-1 w-full">
    <section class="max-w-4xl mx-auto px-4 lg:px-6 py-10">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold text-slate-900">Tambah Tugas Baru</h1>
      ...
      <div class="bg-white rounded-3xl shadow-[0_14px_40px_rgba(15,23,42,0.08)] border border-slate-100 overflow-hidden">
        <div class="bg-sky-50 border-b border-sky-100 px-6 py-4 flex items-center gap-3">
          ...
          <p class="text-sm text-slate-700 font-medium">Isi informasi tugas dengan lengkap</p>
        </div>

        <div class="px-6 lg:px-8 py-8">
          <form method="POST" action="{{ route('asprak.tasks.store') }}">
            @csrf
            ...
            <div class="mb-6">
              ...
              <textarea id="description" name="description" rows="4" 
                        ...
                        placeholder="Deskripsi detail tentang tugas ini..." required>{{ old('description') }}</textarea>
              @error('description')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
              <div>
                ...
                <input type="date" id="deadline" name="deadline" value="{{ old('deadline') }}" min="{{ date('Y-m-d') }}"
            ...
            <div class="mb-6">
              ...
              <p class="mt-1.5 text-xs text-slate-500">Kategori membantu Anda mengelompokkan tugas berdasarkan jenis pekerjaan.</p>
              @error('category')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <button type="submit" class="inline-flex items-center justify-center text-sm font-medium text-white px-6 py-2.5 rounded-xl bg-teal-500 hover:bg-teal-600 transition shadow-sm">
                <span class="mr-2">💾</span> Simpan Tugas
              </button>
            </div>
          </form>
        </div>
      ...
      <div class="mt-6 bg-blue-50 border border-blue-100 rounded-2xl p-5">
        <div class="flex items-start gap-3">
          ...
          <div>
            <h3 class="text-sm font-semibold text-blue-900 mb-2">Tips Manajemen Tugas</h3>
            <ul class="text-sm text-blue-700 space-y-1.5">
              <li>• Gunakan kategori untuk mengelompokkan tugas sejenis</li>
              <li>• Set prioritas tinggi untuk tugas yang mendesak</li>
              <li>• Tentukan deadline realistis agar tidak terlambat</li>
              <li>• Update status secara berkala untuk tracking progress</li>
            </ul>
          </div>
        </div>
      </div>
