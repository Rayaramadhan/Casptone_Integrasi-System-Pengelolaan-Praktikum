...
<body class="bg-gradient-to-br from-slate-50 via-teal-50/30 to-slate-50 min-h-screen">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-slate-900">Edit Backlog</h1>
                        <p class="text-xs text-slate-500">Perbarui informasi backlog</p>
                    </div>
                </div>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="{{ route('asprak.backlogs.update', $backlog) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg border border-slate-100 p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea id="description" 
                          ...
                          required>{{ old('description', $backlog->description) }}</textarea>
                @error('description')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>

            <!-- Deadline, Assign To & Status -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="date" 
                           id="deadline" 
                           name="deadline" 
                           value="{{ old('deadline', $backlog->deadline->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           id="assign_to" 
                           name="assign_to" 
                           value="{{ old('assign_to', $backlog->assign_to) }}"
                ...
                <div>
                    ...
                    <select id="status" 
                            name="status"
                            class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500 @error('status') border-red-500 @enderror"
                            required>
                        <option value="belum_dikerjakan" {{ old('status', $backlog->status) == 'belum_dikerjakan' ? 'selected' : '' }}>Belum Dikerjakan</option>
                        <option value="on_progress" {{ old('status', $backlog->status) == 'on_progress' ? 'selected' : '' }}>On Progress</option>
                        <option value="done" {{ old('status', $backlog->status) == 'done' ? 'selected' : '' }}>Done</option>
                    </select>
                    @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mb-6">
                ...
                <textarea id="progress_notes" 
                          ...
                          placeholder="Catatan progress atau update...">{{ old('progress_notes', $backlog->progress_notes) }}</textarea>
            ...
            <div class="mb-8">
                ...
                <div class="mb-3 p-4 bg-slate-50 rounded-xl flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">📎</span>
                        <div>
                            <p class="text-sm font-medium text-slate-900">File saat ini:</p>
                            <a href="{{ $backlog->progress_file_url }}" target="_blank" class="text-xs text-teal-600 hover:underline">
                ...
                <div class="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-teal-400 transition">
                    ...
                    <label for="progress_file" class="cursor-pointer">
                        ...
                        <p class="text-sm font-medium text-slate-900 mb-1">Klik untuk upload file baru</p>
        ...
        <div class="mt-6 bg-red-50 border border-red-200 rounded-2xl p-6">
            ...
            <p class="text-sm text-red-700 mb-4">Setelah dihapus, backlog ini tidak dapat dipulihkan kembali.</p>
            <form action="{{ route('asprak.backlogs.destroy', $backlog) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus backlog ini?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-6 py-2 bg-red-600 text-white rounded-xl font-medium hover:bg-red-700 transition">
                    Hapus Backlog
                </button>
            </form>
        </div>
